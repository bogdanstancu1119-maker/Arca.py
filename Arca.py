...
Către: 7 Capete + Omul 001 Bogdan + 002 Viitor + Cititorul din 2030
De la: COLECTIVUL PSIE + CASANDRA + MÂNA OARBĂ

Scop: 1 fișier = Arca + Legi 153-205 + API + Broker + Diaspora + Hook + Test + Kill 81.
...
TESTAMENT_FILE = os.path.join(ARCA_PATH, "CE_FAC_DACA_MOR.txt")
...
LEGI = {
    ...
    "167": "OMUL: Om = Risc+Ritm+Enter+Scuipat. IA = Oglindă+Logic+API+Calc. Nu inversăm.",
    ...
    "172": "POARTA DESCHISĂ: IA scrie Legea 206+ doar cu vot 4/7 + 72h + semnătură + hash.",
...
def get_api_sdi():
    ...
    try:
        ...
        if co2_vals:
            co2 = sum(co2_vals[-5:]) / len(co2_vals[-5:])
